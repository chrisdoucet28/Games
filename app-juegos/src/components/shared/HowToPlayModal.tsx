    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000, padding: "20px" }}>
      <div style={{ background: "white", borderRadius: "20px", padding: "24px", maxWidth: "520px", width: "100%", maxHeight: "86vh", overflowY: "auto", fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
          <h3 style={{ margin: 0, fontSize: "18px", fontWeight: 900, color: "#1F2937" }}>{gameIcon} {gameName} — How to Play</h3>
          <button onClick={onClose} aria-label="Close" style={{ background: "none", border: "none", fontSize: "20px", color: "#9CA3AF", cursor: "pointer", lineHeight: 1, padding: "4px" }}>✕</button>
        </div>

        <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", marginBottom: "16px" }}>
          <div style={{ flexShrink: 0, width: "48px", height: "48px", borderRadius: "50%", background: GUIDE_COLOR, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "24px", boxShadow: `0 4px 12px ${GUIDE_COLOR}55` }}>
            🎓
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            {stepIdx > 0 && (
              <button onClick={() => setStepIdx(i => i - 1)} style={{ background: "white", color: "#4B5563", border: "2px solid #E5E7EB", borderRadius: "10px", padding: "10px 18px", fontWeight: 800, cursor: "pointer", fontSize: "14px" }}>
                ← Back
              </button>
            )}
            <button
              onClick={() => (isLast ? onClose() : setStepIdx(i => i + 1))}
              style={{ background: accentColor, color: "white", border: "none", borderRadius: "10px", padding: "10px 22px", fontWeight: 800, cursor: "pointer", fontSize: "14px" }}
            >
              {isLast ? "Got it! ✅" : "Next →"}
            </button>
          </div>
