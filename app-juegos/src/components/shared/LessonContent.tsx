...
// Presentation step identically and can never drift apart. The print-specific rendering
// (PrintableLesson in LearnScreen.tsx) stays separate — it's deliberately plainer/ink-economical,
// a different design goal from this on-screen card.
      ...
      {lesson.commonMistakes.length > 0 && (
        <div style={{ background: "#FFF7ED", border: "2px solid #FDBA74", borderRadius: "12px", padding: "14px 16px", marginTop: "8px" }}>
          ...
          {lesson.commonMistakes.map((m, i) => (
            <div key={i} style={{ fontSize: "13px", color: "#7C2D12", marginBottom: i < lesson.commonMistakes.length - 1 ? "6px" : 0, lineHeight: 1.5 }}>{renderMistake(m)}</div>
          ))}
        </div>
      )}
    </div>
